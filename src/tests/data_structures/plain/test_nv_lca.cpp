...
namespace {
    ...
    using holder= std::shared_ptr<naive_processor_lca<node_type,size_type,value_type>>;
    ...
    class nv_lca_test: public testing::TestWithParam<std::string> {
    public:
        ...
        void SetUp() override {
            if ( not processors.count(GetParam()) ) {
                ...
                n[key] = s.size() / 2;
                processors[key] =
                        std::make_shared<naive_processor_lca<node_type, size_type, value_type>>(s, w[key]);
    ...
    std::map<std::string,holder> nv_lca_test::processors;
    std::map<std::string,std::shared_ptr<path_query_processor<node_type, size_type, value_type>>> nv_lca_test::raws;
    std::map<std::string,size_type> nv_lca_test::n;
    std::map<std::string,std::vector<value_type>> nv_lca_test::w;
    std::map<std::string,std::vector<value_type>> nv_lca_test::sorted_weights;
    std::map<std::string,std::default_random_engine> nv_lca_test::gen_nodes, nv_lca_test::gen_weights, nv_lca_test::gen_qntl;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<node_type>>> nv_lca_test::nodes_distr;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<size_type>>> nv_lca_test::qntl_distr;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<value_type>>> nv_lca_test::weights_distr;

    TEST_P(nv_lca_test,all_queries_correct) {
...
INSTANTIATE_TEST_SUITE_P(testSuiteP,
                         ...
                         [](const testing::TestParamInfo<nv_lca_test::ParamType>& info) {
