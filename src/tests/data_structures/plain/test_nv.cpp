...
namespace {
    ...
    using holder= std::shared_ptr<naive_processor<node_type,size_type,value_type>>;
    ...
    class nv_test: public testing::TestWithParam<std::string> {
    public:
        ...
        void SetUp() override {
            if ( not processors.count(GetParam()) ) {
                ...
                n[key] = s.size() / 2;
                processors[key] =
                        std::make_shared<naive_processor<node_type, size_type, value_type>>(s, w[key]);
    ...
    std::map<std::string,holder> nv_test::processors;
    std::map<std::string,std::shared_ptr<path_query_processor<node_type, size_type, value_type>>> nv_test::raws;
    std::map<std::string,size_type> nv_test::n;
    std::map<std::string,std::vector<value_type>> nv_test::w;
    std::map<std::string,std::vector<value_type>> nv_test::sorted_weights;
    std::map<std::string,std::default_random_engine> nv_test::gen_nodes, nv_test::gen_weights, nv_test::gen_qntl;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<node_type>>> nv_test::nodes_distr;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<size_type>>> nv_test::qntl_distr;
    std::map<std::string,std::shared_ptr<std::uniform_int_distribution<value_type>>> nv_test::weights_distr;

    TEST_P(nv_test,all_queries_correct) {
...
INSTANTIATE_TEST_SUITE_P(testSuiteP,
                         ...
                         [](const testing::TestParamInfo<nv_test::ParamType>& info) {
