...
namespace {
    ...
    using holder= std::unique_ptr<naive_processor<node_type,size_type,value_type>>;

    class nv_test: public testing::Test {
    protected:
        size_type n;
        ...
        void SetUp() override {
            ...
            n= s.size()/2;
            processor=
                    std::make_unique<naive_processor<node_type,size_type,value_type>>(s,w);
    ...
    TEST_F(nv_test,counting_correct) {
    ...
    TEST_F( nv_test, median_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
        for ( auto it= 0; it < ITERS; ++it ) {
            auto x= nodes_dice(), y= nodes_dice();
            assert( this->w.size() >= this->n );
    ...
    TEST_F( nv_test, selection_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
