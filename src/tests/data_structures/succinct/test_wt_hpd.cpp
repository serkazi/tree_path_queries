...
namespace {
    ...
    using wt_hpd_holder= std::unique_ptr<wt_hpd<node_type,size_type,value_type,bp_tree_sada<node_type,size_type>,sdsl::rrr_vector<>>>;

    TEST(wt_hpd,simple_constructor_destructor_sanity_check) {
        std::string s("(((()))()(()))");
        std::vector<value_type> w= {1,2,3,4,5,6,7};
        auto processor=
                std::make_unique<wt_hpd<node_type,size_type,value_type,bp_tree_sada<node_type,size_type>,sdsl::rrr_vector<>>>(s,w);
        processor= nullptr;
    }

    class wthpd_test: public testing::Test {
    protected:
        size_type n;
        ...
        wt_hpd_holder processor= nullptr;
    ...
    TEST_F(wthpd_test,counting_correct) {
    ...
    TEST_F( wthpd_test, median_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
        for ( auto it= 0; it < ITERS; ++it ) {
            auto x= nodes_dice(), y= nodes_dice();
            assert( this->w.size() >= this->n );
    ...
    TEST_F( wthpd_test, selection_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
