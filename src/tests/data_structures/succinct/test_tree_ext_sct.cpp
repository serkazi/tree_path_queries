...
namespace {
    ...
    using holder= std::unique_ptr<tree_ext_sct<node_type,size_type,value_type>>;

    TEST(wt_hpd,simple_constructor_destructor_sanity_check) {
        std::string s("(((()))()(()))");
        std::vector<value_type> w= {1,2,3,4,5,6,7};
        auto processor=
                std::make_unique<tree_ext_sct<node_type,size_type,value_type>>(s,w);
        processor= nullptr;
    }

    class tree_ext_sct_test: public testing::Test {
    protected:
        size_type n;
    ...
    TEST_F(tree_ext_sct_test,counting_correct) {
    ...
    TEST_F( tree_ext_sct_test, median_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
        for ( auto it= 0; it < ITERS; ++it ) {
            auto x= nodes_dice(), y= nodes_dice();
            assert( this->w.size() >= this->n );
    ...
    TEST_F( tree_ext_sct_test, selection_is_correct ) {
        auto nodes_dice= std::bind(*(this->nodes_distr),this->gen_nodes);
