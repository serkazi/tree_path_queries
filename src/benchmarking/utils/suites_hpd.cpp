...
namespace experiment_settings {
    ...
    struct shared_info {
		...
		std::unique_ptr<wt_hp_ptr> decomposer;
        ...
        shared_info() {
			...
			decomposer= std::make_unique<wt_hp_ptr>(topology,w);
			...
			std::for_each(cnt_queries.begin(),cnt_queries.end(),[&counts,this] ( const auto &q ) {
				++counts[decomposer->get_decomposition_length(q.x_,q.y_)];
			});
			/*
			...
			*/
			std::vector<path_queries::counting_query<node_type,size_type,value_type>> selected_queries{};
			std::copy_if(cnt_queries.begin(),cnt_queries.end(),std::back_inserter(selected_queries),[this]( const auto &z ) {
					return decomposer->get_decomposition_length(z.x_,z.y_) == reqlen;
			});
			{	
				auto nn= selected_queries.size();
				std::default_random_engine gg;
				std::uniform_int_distribution<int> dd(0,nn-1);
				cnt_queries.clear();
				for ( auto ll= 0; ll < nq; ++ll ) 
					cnt_queries.push_back(selected_queries[dd(gg)]);
			}

			std::vector<path_queries::median_query<node_type,size_type,value_type>> sq{};
			std::copy_if(med_queries.begin(),med_queries.end(),std::back_inserter(sq),[this]( const auto &z ) {
					return decomposer->get_decomposition_length(z.x_,z.y_) == reqlen;
			});
			{	
				auto nn= sq.size();
				...
				med_queries.clear();
				for ( auto ll= 0; ll < nq; ++ll ) 
					med_queries.push_back(sq[dd(gg)]);
			}
        }
...
void RunAllGiven( int argc, char **argv ) {
    ...
    const int FULL_PATH= 0, NUM_QUERIES= 1, K_VAL= 2, REQLEN= 3;
    ...
    experiment_settings::reqlen 	 = strtol(argv[REQLEN],nullptr,10);
    ...
    std::cerr << "reqlen= " << experiment_settings::reqlen << std::endl;
